...
- M4 🚧 alerts engine (Phase A) — arq cron `evaluate_alerts` every minute; rule types `price_above` / `price_below` / `price_change_pct` / `whale_transfer`; Telegram + HMAC-signed webhook delivery; `/api/alerts/{rules,events}` CRUD; dashboard events panel. See `docs/telegram-setup.md`. Phase B adds rule-creation UI + exchange-netflow rules.
