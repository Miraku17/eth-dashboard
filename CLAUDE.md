...
- 🟡 **On-chain perp positions / leverage / liquidations** — track long/short opens, leverage size, and liquidation events for on-chain perps (GMX V2, Vertex, Aevo, dYdX). Blocker: every meaningful on-chain perp lives on Arbitrum, not mainnet — needs a sibling realtime listener pointed at an Arbitrum WS endpoint (either self-hosted Nitro alongside the existing Geth+Lighthouse pair, or third-party RPC like Alchemy/Ankr free tier). Recommended scope for v1: GMX V2 only, third-party RPC, one new `onchain_perp_event` table (open / close / liquidation), one panel showing live longs/shorts/liquidations with leverage. Hyperliquid + Vertex slot in once the Arbitrum listener exists.
