...
- M3 ✅ whale tracking — Alchemy WS listener persists ETH + 12 stables (USDT/USDC/DAI/PYUSD/FDUSD/USDS/GHO/EUROC/ZCHF/EURCV/EURe/tGBP) transfers above threshold to `transfers`; non-USD stables (EUROC, ZCHF) use `price_usd_approx` per token for FX-aware thresholds; `/api/whales/transfers` exposes them with CEX labels; live-refreshing panel. Needs `ALCHEMY_API_KEY`; thresholds via `WHALE_ETH_THRESHOLD` / `WHALE_STABLE_THRESHOLD_USD`. **After deploy:** re-paste `backend/dune/stablecoin_supply.sql` into the existing `DUNE_QUERY_ID_STABLECOIN_SUPPLY` query on Dune and click Save so the supply panel covers the new stables.
