...
- v2-order-flow ✅ WETH buy vs sell pressure across major DEXes, persists hourly to `order_flow`; `/api/flows/order-flow` endpoint; dashboard panel with buy/sell/net tiles + signed-stacked bar + net line, plus a per-DEX breakdown table (Uniswap V2/V3, Curve, Balancer, Other). **v4 partial migration (2026-05-04):** Uniswap V2 and V3 rows now flow from the realtime listener — `OrderFlowAggregator` decodes Swap events from a curated 8-pool registry (3 V2 + 5 V3) and flushes hourly with additive on_conflict semantics. Curve / Balancer / Other rows remain on Dune (their swap-event ABIs differ from Uniswap and need separate decoders — cards 10b/c). The Dune cron's `upsert_order_flow` now skips uniswap_v2 / uniswap_v3 to avoid overwriting live data. Net Dune savings: roughly half of the 8h order_flow query — a credit-budget improvement, full migration pending. SQL at `backend/dune/order_flow.sql` retained for the Curve/Balancer slice until those decoders ship.
