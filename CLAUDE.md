...
- 🟡 **EUROS clarification** — operator listed `EUROS` in the stablecoin coverage request. Etherscope already tracks 15 stables (USDT/USDC/DAI/PYUSD/FDUSD/USDS/GHO/EUROC/ZCHF/EURCV/EURe/tGBP/USDe/XSGD/BRZ). EUROS is most likely a typo for **EURS** (Stasis Euro). Confirm at next sync; if EURS, add to `STABLES_BY_ADDRESS` in `backend/app/realtime/tokens.py`, bump migrations / Dune supply query.
- 🟡 **Mantle "DEX flows" clarification** — operator listed `MANTEL` alongside Uniswap and Aave under "DEX flows". Two readings: (a) Mantle Network DEX activity (separate L2 chain — needs the same Arbitrum-style sibling listener pointed at Mantle WS), (b) mETH (Mantle Liquid Staking, already tracked under v3-lst). If (a), separate card with same shape as the on-chain perp Arbitrum-listener work. If (b), already shipped — nothing to do.
