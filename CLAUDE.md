...
- 🟡 **Mantle Network DEX flows** — operator confirmed (2026-05-06) the request is reading (a): Mantle Network DEX activity, not mETH (mETH is already shipped under v3-lst). Needs a sibling realtime listener pointed at a Mantle WS endpoint, with its own per-DEX `OrderFlowAggregator` registry. Same shape as the GMX V2 Arbitrum-listener work — likely shippable as a single foundation that swaps the chain config (RPC URL + native-token + token registry + DEX pool registry) for both extensions.
