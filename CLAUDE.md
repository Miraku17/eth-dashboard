...
- M2 ✅ on-chain flows (3 Dune queries → `/api/flows/{exchange,stablecoins,onchain-volume}` → panels). Requires Dune query IDs in `.env` (see `docs/dune-setup.md`); panels show "no data yet" gracefully when unset. **v4 update (2026-05-04):** `/api/flows/onchain-volume` migrated off Dune — endpoint now rolls up the live `realtime_volume` table (per-minute, populated by the realtime listener) into hourly buckets via SQL `date_trunc`. The Dune query is preserved in `DUNE_QUERY_ID_ONCHAIN_VOLUME` for rollback but no longer executed; saves ~720 Dune executions/month.
