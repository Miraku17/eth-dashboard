...
// On DEX, wallet → DEX = selling (red), DEX → wallet = buying (green).
const SELL_COLOR = "rgb(255 92 98)";
const BUY_COLOR = "rgb(25 195 125)";
const NET_COLOR = "rgb(99 102 241)";

export default function DexFlowCurvePanel() {
  const t = useT();
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["dex-series", bucket, asset],
    queryFn: () => fetchDexSeries(bucket, { assets: assetsParam }),
  ...
  const lines: CurveLine[] = [
    { key: "inflow", label: t("dex-flow-curve.line.sell"), color: SELL_COLOR, width: 1.5 },
    { key: "outflow", label: t("dex-flow-curve.line.buy"), color: BUY_COLOR, width: 1.5 },
    { key: "net", label: t("flow-curve.line.net"), color: NET_COLOR, width: 2 },
  ];

  return (
    <Card
      title={t("dex-flow-curve.title")}
      subtitle={t("dex-flow-curve.subtitle", { bucket })}
      live
      actions={
    ...
    >
      <CurvePanelShell
        ...
        emptyHint={t("dex-flow-curve.empty")}
        tiles={
          <Tiles
            totalIn={totalIn}
            totalOut={totalOut}
...
function Tiles({
  ...
  const t = useT();
  // DEX net positive = more selling than buying = bearish.
  const netUp = (lastNet ?? 0) <= 0;
  const netTint =
    lastNet === undefined ? "text-slate-400" : netUp ? "text-up" : "text-down";
  const trendTint =
    trendPct === null
      ? "text-slate-400"
      : Math.abs(trendPct) < 5
  ...
  return (
    <div className="grid grid-cols-2 @sm:grid-cols-4 gap-3">
      ...
      <Tile
        ...
        valueClass={netTint}
        hint={t("dex-flow-curve.tile.net_hint")}
      />
      <Tile
